...
class _MealScheduleViewState extends State<MealScheduleView> {
  ...
  List<Map<String, dynamic>> _mealsForSelectedDate =
      []; // Dữ liệu món ăn của ngày được chọn
  ...
  void _updateMealsForSelectedDate(DateTime selectedDate) {
    setState(() {
      ...
      groupedMeals = groupBy(
        ...
        (meal) =>
            meal['type_meal_name'] as String? ??
            'Unknown', // Nếu không có type_meal_name, gán 'Unknown'
      );
  ...
  Future<void> fetchMeals() async {
    try {
      ...
      final mealsDataRaw =
          await ApiService().fetchMealSchedule(25); // dang nhap bang TK demo@
      ...
      Map<String, List<Map<String, dynamic>>> fetchedGroupedMeals = groupBy(
        mealsData,
        (meal) => meal['type_meal_name'] as String,
      );
      ...
      print("_updateMealsForSelectedDate:" +
          _updateMealsForSelectedDate.toString());
    } catch (e) {
      print('Error fetching meals or type meals: $e');
    }
  }

  Future<void> deleteMealSchedule(int mealScheduleId) async {
    final url = Uri.parse(
        'https://d1e9-203-205-32-22.ngrok-free.app/delete_meal_schedule.php');
    try {
      final response = await http.post(
        url,
        body: {'id': mealScheduleId.toString()},
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Meal schedule deleted successfully')),
          );
        } else {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    'Failed to delete meal schedule: ${responseData['message']}')),
          );
        }
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    return await showDialog<bool>(
          ...
          builder: (BuildContext context) {
            return AlertDialog(
              ...
              content:
                  Text('Are you sure you want to delete this meal schedule?'),
              actions: <Widget>[
                TextButton(
                  child: Text('Cancel'),
                  onPressed: () {
  ...
  Widget build(BuildContext context) {
    ...
    List<Map<String, dynamic>> selectedFoods = _mealsForSelectedDate.isNotEmpty
        ? _mealsForSelectedDate
            .expand((entry) =>
                List<Map<String, dynamic>>.from(entry['foods'] ?? []))
            .toList()
        : [];
    double totalCalories = allFoods.fold<double>(
      ...
      (sum, item) =>
          sum +
          (double.parse(((item['calories_per_unit'] as int).toDouble() *
                            ...
                            child: ElevatedButton(
                              onPressed: () async {
                                ...
                                if (mealTypeId == 0) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    SnackBar(
                                        content: Text('Invalid meal type!')),
                                  );
                                  return;
                                }
